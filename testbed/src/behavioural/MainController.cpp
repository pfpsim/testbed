...
MainController::MainController(sc_module_name nm, pfp::core::PFPObject* parent,std::string configfile ):MainControllerSIM(nm,parent,configfile) {  // NOLINT
  /*sc_spawn threads*/
  ThreadHandles.push_back(
    sc_spawn(sc_bind(&MainController::MainControllerThread, this, 0)));
  ThreadHandles.push_back(
    sc_spawn(sc_bind(&MainController::command_processing_thread, this)));
}

void MainController::init() {
  init_SIM(); /* Calls the init of sub PE's and CE's */
  ControlPlane* cp = dynamic_cast<ControlPlane*>(this->parent_);
  top* t = static_cast<top*>(cp->getParent());
  t->registerControlPlaneToDebugger(this);
}
void MainController::MainController_PortServiceThread() {
  // Thread function to service input ports.
}
void MainController::MainControllerThread(std::size_t thread_id) {
...
// Service method for the Load Balancer service
std::shared_ptr<pfp::cp::CommandResult>
MainController::send_command(const std::shared_ptr<pfp::cp::Command> & cmd) {
  return cpa->send_command(cmd);
}

void MainController::do_command(std::string cmd_str) {
  pfp::cp::CommandParser parser;
  auto cmd = parser.parse_line(cmd_str);
  command_queue.push(cmd);
}

void MainController::command_processing_thread() {
  while (1) {
...
/*
 * The MainController uses the CommandProcessor and ResultProcessor interfaces to notify the debugger about changes
 */
// For pfp::cp::CommandProcessor
std::shared_ptr<pfp::cp::CommandResult> MainController::process
...
std::shared_ptr<pfp::cp::CommandResult>
MainController::process(pfp::cp::ModifyCommand *cmd) {
  std::string table_name = cmd->get_table_name();
...
std::shared_ptr<pfp::cp::CommandResult>
MainController::process(pfp::cp::DeleteCommand *cmd) {
  deleteTableEntry(cmd->get_table_name(), cmd->get_handle());
  return nullptr;
}
std::shared_ptr<pfp::cp::CommandResult>
MainController::process(pfp::cp::BootCompleteCommand*) {
  return nullptr;
}

// For cp::ResultProcessor
void MainController::process(pfp::cp::InsertResult *res) {
...
void MainController::process(pfp::cp::ModifyResult *res) {
...
void MainController::process(pfp::cp::DeleteResult *res) {
...
void MainController::process(pfp::cp::FailedResult*) {
  // TODO(gordon)
}
